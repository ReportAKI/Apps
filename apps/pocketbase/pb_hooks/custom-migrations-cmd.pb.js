...
/* 
    - Usage: ./pocketbase reportaki migrations:revert migration1.js migration2.js ... - reverts specific migrations by name (not by count like the built-in "down" command)
    - Usage: ./pocketbase reportaki migrations:up - runs migrations up and exits with code 1 if any migration fails
*/

// Create a new "reportaki" command group with "migrations:revert" and "migrations:up" subcommands
const reportakiCmd = new Command({
    use: "reportaki",
...
reportakiCmd.addCommand(new Command({
    use: "migrations:up",
...
reportakiCmd.addCommand(new Command({
    ...
    run: exitOnError((_cmd, args) => {
        if (args.length === 0) {
            console.log("Usage: reportaki migrations:revert <migration1.js> [migration2.js] ...")
...
$app.rootCmd.addCommand(reportakiCmd)
